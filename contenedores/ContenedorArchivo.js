...
class Contenedor {
    ...
    async getAll() {
        try {
            const result = await fs.promises.readFile(this.rutaArchivo, 'utf-8');
            const data = JSON.parse(result);
            return data.map(obj => new this.dto(obj));
        } catch(error) {
            logger.error(error);
        }
    }
